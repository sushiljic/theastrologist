...
(function($) {
				...
				/*
				 * Fuel UX Checkbox https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var Checkbox = function(element, options) {
					this.options = $
					.extend({}, $.fn.checkbox.defaults, options);
					...
					toggleContainer : function() {
						if (this.$toggleContainer) {
							if (this.isChecked()) {
								...
								this.$toggleContainer.attr('aria-hidden',
									'false');
							} else {
			...
			(function($) {
				...
				/*
				 * Fuel UX Combobox https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var Combobox = function(element, options) {
					...
					this.$element.on('click', 'a', $.proxy(this.itemclicked,
						this));
					this.$element.on('change', 'input', $.proxy(
				...
				Combobox.prototype = {
					...
					itemclicked : function(e) {
						...
						// set input text and trigger input change event marked
						// as synthetic
						this.$input.val(this.$selectedItem.text()).trigger(
							'change', {
								synthetic : true
							});
					...
					inputchanged : function(e, extra) {
						...
						// skip processing for internally-generated synthetic
						// event
				...
				$('body').on('mousedown.combobox.data-api', '.combobox',
					function() {
						var $this = $(this);
			...
			(function($) {
				...
				/*
				 * Fuel UX Datepicker https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				// only load moment if it's there. otherwise we'll look for it
				// in window.moment
				// you need to make sure moment is loaded before the rest of
				// this module
				...
				if (typeof define === 'function' && define.amd) {
					require(
						...
						function(err) {
							var failedId = err.requireModules
							&& err.requireModules[0];
							if (failedId === 'moment') {
									// do nothing cause that's the point of
									// progressive enhancement
									if (typeof window.console !== 'undefined') {
										if (window.navigator.userAgent
											.search('PhantomJS') < 0) {
											// don't show this in phantomjs
											// tests
											window.console
											.log("Don't worry if you're seeing a 404 that's looking for moment.js. The Fuel UX Datepicker is trying to use moment.js to give you extra features.");
											window.console
											.log("Checkout the Fuel UX docs (http://exacttarget.github.io/fuelux/#datepicker) to see how to integrate moment.js for more features");
										}
									}
								}
							});
				...
				var Datepicker = function(element, options) {
					...
					if (this._checkForMomentJS()) {
						moment = moment || window.moment; // need to pull in
															// the global moment
															// if they didn't do
															// it via require
															this.moment = true;
				...
				Datepicker.prototype = {
					...
					disable : function() {
						this.$element.find('input, button').attr('disabled',
							true);
					...
					formatDate : function(date) {
						// if we have moment available use it to format dates.
						// otherwise use default
						if (Boolean(this.moment)) {
							return moment(date).format(this.momentFormat);
						} else {
							// this.pad to is function on extension
							return this.padTwo(date.getMonth() + 1) + '-'
					...
					parseDate : function(date, silent) {
						...
						if (this.moment) {
							silent = silent || false;
							// if silent is requested (direct user input
							// parsing) return true or false not a date object,
							...
							if (silent) {
								...
								return moment(date).toDate(); // example of
																// using moment
																// for parsing
															}
														} else {
							// if moment isn't present, use previous date
							// parsing strategry
							...
							if (Boolean(date)
								...
								if (typeof (date) === 'string') {
									...
									if (parts) {
										month = +parts[2];
										dt.setFullYear(parts[1], month - 1,
											parts[3]);
					...
					_restrictDateSelectionSetup : function() {
						...
						if (Boolean(this.options)) {
							if (!this.options.restrictDateSelection) {
								...
								scopedNextMonth = (this.viewDate.getMonth() < new Date()
									.getMonth()) ? true : false;
								scopedLastMonth = (this.viewDate.getMonth() > new Date()
									.getMonth()) ? false : true;
							}
						}
					...
					_processDateRestriction : function(date, returnClasses) {
						...
						if (this.options.restrictToYear
							&& this.options.restrictToYear !== date
							.getFullYear()) {
							classes += ' restrict';
						...
						if (Boolean(this.blackoutDates(date))) {
							...
							restrictBoolean = true;
						} else if (Boolean(this.options)
							&& Boolean(this.options.restrictDateSelection)) {
							classes += ' restrict';
							restrictBoolean = true;
						} else {
							classes += ' past';
				...
				_updateCalendarData : function() {
					...
					var firstDayOfMonthWeekday = new Date(viewedYear,
						viewedMonth, 1).getDay();
					var lastDayOfMonth = this._getDaysInMonth(viewedMonth,
						viewedYear);
					var lastDayOfLastMonth = this._getDaysInMonth(
						viewedMonth - 1, viewedYear);

					if (firstDayOfMonthWeekday === 0) {
						firstDayOfMonthWeekday = 7;
					}

					var addToEnd = (42 - lastDayOfMonth)
					- firstDayOfMonthWeekday;

					this.daysOfLastMonth = this._range(lastDayOfLastMonth
						- firstDayOfMonthWeekday + 1,
						lastDayOfLastMonth + 1);
					this.daysOfNextMonth = this._range(1, addToEnd + 1);

						// blackout functionality for dates of last month on
						...
						for (var x = 0, xx = this.daysOfLastMonth.length; x < xx; x++) {
							...
							tmpLastMonthDaysObj['class'] = this
							._processDateRestriction(
								new Date(viewedYear,
									viewedMonth + 1,
									this.daysOfLastMonth[x], 0,
									0, 0, 0), true);
							tmpLastMonthDaysObj['class'] += ' past';
							this.daysOfLastMonth[x] = tmpLastMonthDaysObj;
						}

						// blackout functionality for dates of next month on
						// current calendar view
						for (var b = 0, bb = this.daysOfNextMonth.length; b < bb; b++) {
							...
							tmpNextMonthDaysObj['class'] = this
							._processDateRestriction(
								new Date(viewedYear,
									viewedMonth + 1,
									this.daysOfNextMonth[b], 0,
									0, 0, 0), true);
						...
						var daysOfThisMonth = this
						._range(1, lastDayOfMonth + 1);
						this.daysOfThisMonth = [];

						for (var i = 0, ii = daysOfThisMonth.length; i < ii; i++) {

							var weekDay = new Date(viewedYear, viewedMonth,
								daysOfThisMonth[i]).getDay();
							...
							if (daysOfThisMonth[i] === selectedDay
								&& viewingSelectedMonth
								&& viewingSelectedYear) {
								weekDayClass += ' selected';
						} else if (daysOfThisMonth[i] === currentDay
							&& viewingCurrentMonth
							&& viewingCurrentYear) {
							weekDayClass += ' today';
						}

						var dt = new Date(viewedYear, viewedMonth,
							daysOfThisMonth[i], 0, 0, 0, 0);
						weekDayClass += this._processDateRestriction(dt,
							true);
					...
					var daysInMonth = this._getDaysInMonth(this.minDate
						.getFullYear(), this.minDate.getMonth());
					for (var j = 0, jj = this.months.length; j < jj; j++) {
						...
						var minDt = new Date(viewedYear, j, daysInMonth,
							23, 59, 59, 999);
					...
					this.years = this._yearRange(this.viewDate);
					daysInMonth = this._getDaysInMonth(this.minDate
						.getFullYear(), 11);
					...
					for (var z = 0, zz = this.years.length; z < zz; z++) {
						...
						var minDt2 = new Date(this.years[z].number, 11,
							daysInMonth, 23, 59, 59, 999);
						var maxDt2 = new Date(this.years[z].number, 0, 0,
							0, 0, 0, 0);
						if (minDt2 <= this.minDate
							|| maxDt2 >= this.maxDate) {
							if (Boolean(this.options.restrictDateSelection)) {
				...
				_updateCss : function() {
					...
					this.$view.css('width', this.options.dropdownWidth
						+ 'px');
					this.$header.css('width', this.options.dropdownWidth
						+ 'px');
					this.$labelDiv.css('width',
						(this.options.dropdownWidth - 60) + 'px');
					this.$footer.css('width', this.options.dropdownWidth
						+ 'px');
					var labelSize = (this.options.dropdownWidth * 0.25);
					var paddingTop = Math
					.round((this.options.dropdownWidth - (labelSize * 3)) / 2);
					...
					var cellSize = Math
					.round(this.options.dropdownWidth / 7.0)
					+ 'px';
					var headerCellSize = Math
					.round(this.options.dropdownWidth / 7.0)
					+ 'px';
					this._applySize(this.$yearsView.children(), labelSize
						+ 'px');
					this._applySize(this.$monthsView.children(), labelSize
						+ 'px');
					this._applySize(this.$weekdaysDiv.children(),
						headerCellSize);
					this
					._applySize(this.$lastMonthDiv.children(),
						cellSize);
					this
					._applySize(this.$thisMonthDiv.children(),
						cellSize);
					this
					._applySize(this.$nextMonthDiv.children(),
						cellSize);
				},
				...
				_select : function(e) {
					...
					this.stagedDate
					.setDate(parseInt(e.target.innerHTML, 10));
				...
				_pickMonth : function(e) {
					var month = parseInt($(e.target).data('monthNumber'),
						10);
					if (e.target.className.indexOf('restrict') > -1) {
						return this._killEvent(e);
					}

					this.viewDate = new Date(this.viewDate.getFullYear(),
						month, 1);
					this._showView(1);
					this._render();
				...
				_previous : function(e, set) {
					...
					if (this.options.showDays) {
						this.viewDate = new Date(this.viewDate
							.getFullYear(),
							this.viewDate.getMonth() - 1, 1);
					} else if (this.options.showMonths) {
						this.viewDate = new Date(this.viewDate
							.getFullYear() - 1, this.viewDate
							.getMonth(), 1);
					} else if (this.options.showYears) {
						this.viewDate = new Date(this.viewDate
							.getFullYear() - 10, this.viewDate
							.getMonth(), 1);
					}
					...
					if (Boolean(set)) {
						...
						// move this below 'this._render()' if you want it to go
						// to the previous month when you select a day from the
						// current month
						return this._killEvent(e);
					},
					...
					_next : function(e, set) {
						...
						// move this below 'this._render()' if you want it to go
						// to the next month when you select a day from the
						// current month
						return this._killEvent(e);
					},
					...
					_monthYearLabel : function() {
						var label;
						if (this.options.showDays) {
							label = this._monthLabel() + ' '
							+ this._yearLabel();
						} else if (this.options.showMonths) {
							label = this._yearLabel();
						} else if (this.options.showYears) {
							label = this.years[0].number + ' - '
							+ this.years[this.years.length - 1].number;
						}
						return label;
					},
					...
					_renderCalendar : function() {
						...
						+ self._monthYearLabel()
						+ '</div>'
						+ '</div>'
						+ '<div class="daysView" style="'
						+ self._show(self.options.showDays)
						+ '">'
						+

						self._repeat('<div class="weekdays">',
							self.options.weekdays,
							function(weekday) {
								return '<div >' + weekday
								+ '</div>';
							}, '</div>')
						...
						self._repeat('<div class="thismonth">',
							self.daysOfThisMonth, function(day) {
								return '<div class="'
								+ day['class'] + '">'
								+ day.number + '</div>';
							}, '</div>')
						...
						self._repeat('<div class="monthsView" style="'
							+ self._show(self.options.showMonths)
							+ '">', self.months, function(month) {
								return '<div data-month-number="'
								+ month.number + '" class="'
								+ month['class'] + '">'
								+ month.abbreviation + '</div>';
							}, '</div>')
						+

						self._repeat('<div class="yearsView" style="'
							+ self._show(self.options.showYears)
							+ '">', self.years, function(year) {
								return '<div data-year-number="'
								+ year.number + '" class="'
								+ year['class'] + '">'
								+ year.number + '</div>';
							}, '</div>') +
					...
					_calculateInputSize : function(options) {
						if (Boolean(parseInt(
							this.options.createInput.inputSize, 10))) {
							return 'style="width:'
						+ this.options.createInput.inputSize
						+ 'px"';
					} else {
				...
				_inputDateParsing : function() {
						// the formats we support when using moment.js are
						// either "L" or "l"
						// these can be found here
						// http://momentjs.com/docs/#/customization/long-date-formats/
						var inputValue = this.$input.val();
						var triggerError = true;
						var validLengthMax = 10; // since the length of the
													// longest date format we
													// are going to parse is 10
													// ("L" format code) we will
													// set this here.
						var validLengthMin = validLengthMax - 2; // since the
																	// shortest
																	// date
																	// format we
																	// are going
																	...
																	if (inputValue.length >= validLengthMin
																		&& inputValue.length <= validLengthMax) {
																		if (Boolean(this.parseDate(inputValue, true))) {
																			if (!this._processDateRestriction(this
							...
							triggerError = false; // don't want to trigger an
					...
					_checkForMomentJS : function() {
						// this function get's run on initialization to determin
						// if momentjs is available
						if ($.isFunction(window.moment)
							|| (typeof moment !== "undefined" && $
								.isFunction(moment))) {
							if ($.isPlainObject(this.options.momentConfig)) {
								if (Boolean(this.options.momentConfig.culture)
									&& Boolean(this.options.momentConfig.formatCode)) {
									return true;
							} else {
								return false;
							}
				...
				_addBindings : function() {
					var self = this;
						...
						// parsing dates on user input is only available when
						// momentjs is used
						if (Boolean(this.moment)) {
						...
						this.$calendar.on('click', $.proxy(this._emptySpace,
							this));

						if (this.options.restrictToYear !== this.viewDate
							.getFullYear()
							|| this.viewDate.getMonth() > 0) {
							this.$header.find('.left').on('click',
								$.proxy(this._previous, this));
					} else {
					...
					if (this.options.restrictToYear !== this.viewDate
						.getFullYear()
						|| this.viewDate.getMonth() < 11) {
						this.$header.find('.right').on('click',
							$.proxy(this._next, this));
				} else {
					this.$header.find('.right').addClass('disabled');
				}

				this.$header.find('.center').on('click',
					$.proxy(this._toggleMonthYearPicker, this));

				this.$lastMonthDiv.find('div').on('click',
					$.proxy(this._previousSet, this));
				this.$thisMonthDiv.find('div').on('click',
					$.proxy(this._select, this));
				this.$nextMonthDiv.find('div').on('click',
					$.proxy(this._nextSet, this));

				this.$monthsView.find('div').on('click',
					$.proxy(this._pickMonth, this));
				this.$yearsView.find('div').on('click',
					$.proxy(this._pickYear, this));
				this.$footer.find('.center').on('click',
					$.proxy(this._today, this));

				this.bindingsAdded = true;
			},

			_removeBindings : function() {
						// remove event only if moment is available (meaning it
...
this.bindingsAdded = false;
				...
				$.fn.datepicker.defaults = {
					...
					momentConfig : {
						culture : 'en',
						formatCode : 'L' // more formats can be found here
											// http://momentjs.com/docs/#/customization/long-date-formats/.
											// We only support "L" or "l"
										},
										dropdownWidth : 170,
				...
				$(document.body).on("click",
					"[data-toggle=dropdown][data-flip]", function(event) {

						if ($(this).data().flip === "auto") {
								// have the drop down decide where to place
								// itself
				...
				function dropUpCheck(element) {
					...
					measurements.containerHeight = $container.overflowElement
					.outerHeight();

					// this needs to be different if the window is the container
					// or another element is
					measurements.containerOffsetTop = (!!$container.isWindow) ? $container.overflowElement
					.scrollTop()
					: $container.overflowElement.offset().top;

					// doing the calculations
					measurements.fromTop = measurements.parentOffsetTop
					- measurements.containerOffsetTop;
					measurements.fromBottom = measurements.containerHeight
			...
			(function($) {
				...
				/*
				 * Fuel UX Loader https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var Loader = function(element, options) {
					...
					this.begin = (this.$element.is('[data-begin]')) ? parseFloat(this.$element
						.attr('data-begin'))
					: 1;
					this.delay = (this.$element.is('[data-delay]')) ? parseFloat(this.$element
						.attr('data-delay'))
					: 150;
					this.end = (this.$element.is('[data-end]')) ? parseInt(
						this.$element.attr('data-end'), 10) : 8;
					this.frame = (this.$element.is('[data-frame]')) ? parseInt(
						this.$element.attr('data-frame'), 10) : 1;
					this.timeout = {};
			...
			(function($) {
				...
				/*
				 * Fuel UX Placard https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var Placard = function(element, options) {
					...
					if (this.options.revertOnCancel === -1) {
						this.options.revertOnCancel = (this.$accept.length > 0) ? true
						: false;
					}

					this.$field.on('click', $.proxy(this.show, this));
					this.$accept.on('click', $.proxy(this.complete, this,
						'accept'));
				...
				Placard.prototype = {
					...
					complete : function(action) {
						var func = this.options['on' + action[0].toUpperCase()
						+ action.substring(1)];
						var obj = {
					...
					if (this.$element.attr('data-ellipsis') === 'true') {
						...
						if (this.$field.is('input')) {
							...
							if (field.clientHeight < field.scrollHeight) {
								...
								str = (str.length > 0) ? str.substring(0,
									str.length - 1) : '';
								this.$field.val(str + '...');
							}
						}
			...
			show : function() {
				...
				if (other.length > 0) {
					if (other.data('placard')
						&& other.data('placard').options.explicit) {
						return;
				}
			...
			if (this.$header.length > 0) {
				this.$popup.css('top', '-'
					+ this.$header.outerHeight(true) + 'px');
			}
			if (this.$footer.length > 0) {
				this.$popup.css('bottom', '-'
					+ this.$footer.outerHeight(true) + 'px');
			}
			...
			if (!this.options.explicit) {
				...
				$.fn.placard = function(option) {
					...
					var $set = this.each(function() {
						...
						if (!data)
							$this.data('placard', (data = new Placard(this,
								options)));
						if (typeof option === 'string')
							methodReturn = data[option].apply(data, args);
					});
				...
				$.fn.placard.defaults = {
					...
					explicit : false,
					revertOnCancel : -1
				// negative 1 will check for an '.placard-accept' button. Also
			...
			$.fn.placard.noConflict = function() {
				...
				$('body').on('mousedown.placard.data-api', '.placard',
					function() {
						var $this = $(this);
						if ($this.data('placard'))
							return;
			...
			(function($) {
				...
				/*
				 * Fuel UX Radio https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				Radio.prototype = {
					...
					toggleContainer : function() {
						var group;
						if (this.$toggleContainer) {
							// show corresponding container for currently
							// selected radio
							if (this.isChecked()) {
			...
			(function($) {
				...
				/*
				 * Fuel UX Search https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var Search = function(element, options) {
					...
					this.$button = this.$element.find('button').on('click',
						$.proxy(this.buttonclicked, this));

					this.$input = this.$element.find('input').on('keydown',
						$.proxy(this.keypress, this)).on('keyup',
						$.proxy(this.keypressed, this));
					...
					Search.prototype = {
						...
						action : function() {
							...
							var inputEmptyOrUnchanged = val === ''
							|| val === this.activeSearch;
						...
						keypressed : function(e) {
							...
							if (e.which === 13) {
								...
								val = this.$input.val();
								inputPresentAndUnchanged = val
								&& (val === this.activeSearch);
								this.$icon
								.attr(
									'class',
									inputPresentAndUnchanged ? 'glyphicon glyphicon-remove'
				...
				$.fn.search = function(option) {
					...
					var $set = this.each(function() {
						...
						if (!data)
							$this.data('search', (data = new Search(this,
								options)));
						if (typeof option === 'string')
							methodReturn = data[option].apply(data, args);
					});
				...
				$('body').on('mousedown.search.data-api', '.search',
					function() {
						var $this = $(this);
						if ($this.data('search'))
							return;
			...
			(function($) {
				...
				/*
				 * Fuel UX Button Dropdown https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var Selectlist = function(element, options) {
					this.$element = $(element);
					this.options = $.extend({}, $.fn.selectlist.defaults,
						options);
				...
				Selectlist.prototype = {
					...
					doSelect : function($item) {
						this.$selectedItem = $item;
						this.$hiddenField.val(this.$selectedItem
							.attr('data-value'));
					...
					resize : function() {
						...
						if (Boolean($(document).find('html').hasClass('fuelux'))) {
							// default behavior for fuel ux setup. means fuelux
							// was a class on the html tag
							$(document.body).append(sizer);
						} else {
							// fuelux is not a class on the html tag. So we'll
							// look for the first one we find so the correct
							// styles get applied to the sizer
						...
						// TODO: betting this is somewhat off with box-sizing:
						// border-box
				...
				$.fn.selectlist = function(option) {
					...
					var $set = this.each(function() {
						...
						if (!data)
							$this.data('selectlist', (data = new Selectlist(
								this, options)));
						if (typeof option === 'string')
							methodReturn = data[option].apply(data, args);
					});
				...
				$('body').on('mousedown.fu.selectlist.data-api', '.selectlist',
					function() {
						var $this = $(this);
			...
			(function($) {
				...
				/*
				 * Fuel UX Spinbox https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var Spinbox = function(element, options) {
					...
					this.$element.on('focusin', this.$input, $.proxy(
						this.changeFlag, this));
					this.$element.on('focusout', this.$input, $.proxy(
						this.change, this));

					if (this.options.hold) {
						this.$element.on('mousedown', '.spinbox-up', $.proxy(
							function() {
								this.startSpin(true);
							}, this));
						this.$element.on('mouseup',
							'.spinbox-up, .spinbox-down', $.proxy(
								this.stopSpin, this));
						this.$element.on('mouseout',
							'.spinbox-up, .spinbox-down', $.proxy(
								this.stopSpin, this));
						this.$element.on('mousedown', '.spinbox-down', $.proxy(
							function() {
								this.startSpin(false);
							}, this));
					} else {
						this.$element.on('click', '.spinbox-up', $.proxy(
							function() {
								this.step(true);
							}, this));
						this.$element.on('click', '.spinbox-down', $.proxy(
							function() {
								this.step(false);
							}, this));
					}
				...
				Spinbox.prototype = {
					...
					render : function() {
						...
						this.$input
						.attr('maxlength',
							(this.options.max + maxUnitLength)
							.split('').length);
					},
					...
					change : function() {
						...
						if (this.options.units.length) {
							...
							newVal = this.checkMaxMin(newVal.replace(
								/[^0-9.-]/g, '')
							|| '');
					...
					startSpin : function(type) {
						...
						if (!this.options.disabled) {
							...
							this.switches.timeout = setTimeout($.proxy(
								function() {
									this.iterator(type);
					...
					step : function(dir) {
						...
						if ((dir ? curValue < limValue : curValue > limValue)) {
							var newVal = curValue + (dir ? 1 : -1)
							* this.options.step;

							if (this.options.step % 1 !== 0) {
								digits = (this.options.step + '').split('.')[1].length;
								multiple = Math.pow(10, digits);
								newVal = Math.round(newVal * multiple)
								/ multiple;
							}
							...
							var cycleVal = dir ? this.options.min
							: this.options.max;
							this.value(cycleVal);
						}
					},
...
checkMaxMin : function(value) {
	...
	limit = value >= this.options.max ? this.options.max
	: this.options.min;
	...
	this.$element.find('button').removeClass('disabled');
				...
				$.fn.spinbox = function(option) {
					...
					var $set = this.each(function() {
						...
						if (!data)
							$this.data('spinbox', (data = new Spinbox(this,
								options)));
						if (typeof option === 'string')
							methodReturn = data[option].apply(data, args);
					});
				...
				$('body').on('mousedown.spinbox.data-api', '.spinbox',
					function() {
						var $this = $(this);
			...
			(function($) {
				...
				var Tree = function(element, options) {
					...
					this.$element.on('click', '.tree-item', $.proxy(
						function(ev) {
							this.selectItem(ev.currentTarget);
						}, this));
					this.$element.on('click', '.tree-folder-header', $.proxy(
						function(ev) {
							this.openFolder(ev.currentTarget);
						}, this));

					if (this.options.folderSelect) {
						this.$element.on('click', '.tree-folder-name', $.proxy(
							function(ev) {
...
selectItem : function(el) {
	...
	if ($el.hasClass('tree-selected')) {
		...
		$el.find('i').removeClass('glyphicon-ok').addClass(
			'tree-dot');
	} else {
		$el.addClass('tree-selected');
		$el.find('i').removeClass('tree-dot').addClass(
			'glyphicon-ok');
		if (this.options.multiSelect) {
			data.push($el.data());
		}
		...
		this.$element.trigger('selected', {
			info : data
					...
					openFolder : function(el) {
						var $el = $(el);
						var $parent = $el.parent();
						var $treeFolderContent = $parent
						.find('.tree-folder-content');
						var $treeFolderContentFirstChild = $treeFolderContent
						.eq(0);
						...
						if ($el.find('.glyphicon-folder-close').length) {
							...
							if (this.options.folderSelect) {
								$el.find('.tree-triangle-right').removeClass(
									'tree-triangle-right').addClass(
									'tree-triangle-down');
								}
							} else {
								eventType = 'closed';
								...
								if (this.options.folderSelect) {
									$el.find('.tree-triangle-down').removeClass(
										'tree-triangle-down').addClass(
										'tree-triangle-right');
									}
								}
							...
							selectFolder : function(el) {
								var $el = $(el);
								var $all = this.$element
								.find('.tree-folder-name.tree-selected');
								var data = [];
								var eventType = 'selected';
								...
								if (this.options.multiSelect) {
									$.each($all, function(index, value) {
										var $val = $(value);
										if ($val[0] !== $el[0]) {
											data.push($(value).parent().find(
												'.tree-folder-header').data());
										}
									});
								} else if ($all[0] !== $el[0]) {
									$all.removeClass('tree-selected');
									data.push($el.parent().find('.tree-folder-header')
										.data());
								}
					...
					collapse : function() {
						...
						.each(
							function() {
											...
											var $this = $(this)
											.removeClass(
												'icon-folder-close icon-folder-open')
											.addClass(
												'icon-folder-close');
			...
			(function($) {
				...
				/*
				 * Fuel UX Wizard https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var Wizard = function(element, options) {
					...
					this.$element.on('click', 'li.complete', $.proxy(
					...
					if (this.options.disablePreviousStep) {
						...
						this.$element.find('.steps').addClass(
							'previous-disabled');
					}
				};
				...
				Wizard.prototype = {
					...
					// index is 1 based, remove is how many to remove after
					// adding items
					// third parameter can be array of objects [{ ... }, { ...
					...
					addSteps : function(index, remove) {
						...
						remove = remove || 0;
						index = (index > (this.numSteps + 1)) ? this.numSteps + 1
						: index;
						...
						for (i = 0, l = items.length; i < l; i++) {
							...
							$step.find('.badge')
							.append(items[i].badge || index);

							$pane = $('<div class="step-pane" data-step="'
								+ index + '"></div>');
					...
					setState : function() {
						...
						if (last) {
							this.lastText = last;
							// replace text
							var text = (lastStep !== true) ? this.nextText
							: this.lastText;
...
$currentStep.find('span.badge').addClass('badge-info');
						...
						if (totalWidth > containerWidth) {
							...
							// set the position so that the active step is in a
							// good
							// position if it has been moved out of view
							if (this.$element.find('li.active').first()
								.position().left < 200) {
				...
				selectedItem : function(selectedItem) {
					...
					if (selectedItem) {
						...
						if (step >= 1 && step <= this.numSteps) {
							...
							step = this.$element.find(
								'.steps li.active:first').attr(
								'data-step');
								if (!isNaN(step)) {
				...
				$.fn.wizard = function(option) {
					...
					var $set = this.each(function() {
						...
						if (!data)
							$this.data('wizard', (data = new Wizard(this,
								options)));
						if (typeof option === 'string')
							methodReturn = data[option].apply(data, args);
					});
				...
				// -1 means it will attempt to look for "active" class in order
				// to set the step
			};
			...
			$.fn.wizard.noConflict = function() {
				...
				$('body').on('mouseover.wizard.data-api', '.wizard',
					function() {
						var $this = $(this);
						if ($this.data('wizard'))
							return;
			...
			(function($) {
				...
				/*
				 * Fuel UX Infinite Scroll https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var InfiniteScroll = function(element, options) {
					...
					this.options = $.extend({}, $.fn.infinitescroll.defaults,
						options);
					...
					this.$element.on('scroll.fuelux.infinitescroll', $.proxy(
						this.onScroll, this));
				};
				...
				InfiniteScroll.prototype = {
					...
					enable : function() {
						this.$element.on('scroll.fuelux.infinitescroll', $
					...
					getPercentage : function() {
						var height = (this.$element.css('box-sizing') === 'border-box') ? this.$element
						.outerHeight()
						: this.$element.height();
					...
					fetchData : function(force) {
						...
						if (this.options.hybrid && force !== true) {
							...
							moreBtn.on('click.fuelux.infinitescroll',
								function() {
									moreBtn.remove();
									fetch();
								});
				...
				$.fn.infinitescroll = function(option) {
					...
					.each(function() {
						...
						if (!data)
							$this.data('infinitescroll',
								(data = new InfiniteScroll(this,
									options)));
						if (typeof option === 'string')
				...
				$.fn.infinitescroll.defaults = {
					dataSource : null,
					hybrid : false, // can be true or an object with structure:
									// { 'label': (markup or jQuery obj) }
									percentage : 95
				// percentage scrolled to the bottom before more is loaded
			};
		...
		(function($) {
				...
				/*
				 * Fuel UX Pillbox https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var Pillbox = function(element, options) {
					...
					// EVENTS
					this.acceptKeyCodes = this
					._generateObject(this.options.acceptKeyCodes);
					// Creatie an object out of the key code array, so we dont
					// have to loop through it on every key stroke

					this.$element.on('click', '.pill-group > .pill', $.proxy(
						this.itemClicked, this));
					this.$element.on('click', $.proxy(this.inputFocus, this));
					this.$element.on('keydown', '.pillbox-add-item', $.proxy(
				...
				Pillbox.prototype = {
					...
					items : function() {
						...
						return this.$pillGroup.children('.pill').map(
							function() {
					...
					itemClicked : function(e) {
						...
						if ($text.length
							&& !$target.parent().hasClass('pill-group')) {
						...
						this.$element.trigger('clicked', this
							.getItemData($item));
					},
					...
					// First parameter is 1 based index (optional, if index is
					// not passed all new items will be appended)
					// Second parameter can be array of objects [{ ... }, { ...
					// }] or you can pass n additional objects as args
					// object structure is as follows (index and value are
					// optional): { text: '', value: '' }
addItems : function() {
	var self = this;
	...
	isInternal = items[1] && !items[1].text;
						...
						if (items.length) {
							...
							if (this.options.edit && this.currentEdit) {
								items[0].el = this.currentEdit.wrap(
									'<div></div>').parent().html();
							}
							...
							if (self.options.onAdd && isInternal) {
								...
								if (this.options.edit && this.currentEdit) {
									self.options.onAdd(items[0], $.proxy(
										self.saveEdit, this));
								} else {
									self.options.onAdd(items[0], $.proxy(
										self.placeItems, this, true));
								}
							} else {
								if (this.options.edit && this.currentEdit) {
									self.saveEdit(items);
					...
					// First parameter is the index (1 based) to start removing
					// items
					...
					if (items.length) {
						$.each(items, function(i, item) {
							...
							newHtml += $item.wrap('<div></div>').parent()
							.html();
						});

						if (this.$pillGroup.children('.pill').length > 0) {
							if (index) {
								$neighbor = this.$pillGroup
								.find('.pill:nth-child(' + index
									+ ')');
								...
								if ($neighbor.length) {
									...
									this.$pillGroup.children('.pill:last')
									.after(newHtml);
								}
							} else {
								this.$pillGroup.children('.pill:last')
				...
				inputEvent : function(e) {
					...
					if (this.acceptKeyCodes[e.keyCode]) {
						...
						if (this.options.onKeyDown
							...
							$selection = this.$suggest
						.find('.pillbox-suggest-sel');
					...
					return true;
							...
							if (!text.length) {
								...
								this._closeSuggestions();
								$lastItem = this.$pillGroup
								.children('.pill:last');

								if ($lastItem.hasClass('pillbox-highlight')) {
									this._removeElement(this.getItemData(
										$lastItem, {
											el : $lastItem
										}));
								} else {
							...
							if (this.$addItem.width() < (this.$pillGroup
								.width() - 6)) {
								this.$addItem.attr({
									size : text.length + 3
								});
					...
					this.$pillGroup.find('.pill').removeClass(
						'pillbox-highlight');
					...
					if (this.options.onKeyDown) {
						if (e.keyCode === 9 || e.keyCode === 38
							...
							// only allowing most recent event callback to
							// register
					...
					openEdit : function(el) {
						...
						this.$pillGroup.find('.pill:nth-child(' + index + ')')
						.before($addItemWrap);
					...
					// Must match syntax of placeItem so addItem callback is
					// called when an item is edited
					// expecting to receive an array back from the callback
					// containing edited items
					saveEdit : function() {
						...
						this.$addItem.val('');
						this.$pillGroup.append(this.$addItemWrap.detach()
							.show());
					...
					_keySuggestions : function(e) {
						var $first = this.$suggest
						.find('li.pillbox-suggest-sel');
						...
						if (!$first.length) {
							...
							if (!$next.length) {
								$next = dir ? this.$suggest.find('li:last')
				...
				$.fn.pillbox = function(option) {
					...
					var $set = this.each(function() {
						...
						if (!data)
							$this.data('pillbox', (data = new Pillbox(this,
								options)));
						if (typeof option === 'string')
							methodReturn = data[option].apply(data, args);
					});
				...
				$.fn.pillbox.defaults = {
					...
					onKeyDown : undefined,
					edit : true,
					acceptKeyCodes : [ 13, // Enter
					188 // Comma
					]

				// example on remove
				/*
				 * onRemove: function(data,callback){ console.log('onRemove');
				 * callback(data); }
				 */

				// example on key down
				/*
				 * onKeyDown: function(event, data, callback ){ callback({data:[
				 * {text: Math.random(),value:'sdfsdfsdf'}, {text:
				 * Math.random(),value:'sdfsdfsdf'} ]}); }
*/
				// example onAdd
				/*
				 * onAdd: function( data, callback ){ console.log(data,
...
$.fn.pillbox.noConflict = function() {
	...
	return this;
				...
				$('body').on('mousedown.pillbox.data-api', '.pillbox',
					function() {
						var $this = $(this);
			...
			(function($) {
				...
				/*
				 * Fuel UX Repeater https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var Repeater = function(element, options) {
					...
					this.$pageSize = this.$element
					.find('.repeater-itemization .selectlist');
					this.$nextBtn = this.$element.find('.repeater-next');
					this.$pages = this.$element.find('.repeater-pages');
					this.$prevBtn = this.$element.find('.repeater-prev');
					this.$primaryPaging = this.$element
					.find('.repeater-primaryPaging');
					this.$search = this.$element.find('.repeater-search');
					this.$secondaryPaging = this.$element
					.find('.repeater-secondaryPaging');
					...
					this.options = $
					.extend({}, $.fn.repeater.defaults, options);
					this.staticHeight = (this.options.staticHeight === -1) ? this.$element
					.attr('data-staticheight')
					: this.options.staticHeight;
					...
					this.$prevBtn.on('click', $.proxy(this.previous, this));
					this.$primaryPaging.find('.combobox').on('changed',
						function(evt, data) {
							self.pageInputChange(data.text);
						});
					this.$search.on('searched cleared', $.proxy(this.render,
						this, {
							clearInfinite : true,
							pageIncrement : null
						}));
					...
					this.$views.find('input').on('change',
						$.proxy(this.viewChanged, this));

					this.$loader.loader();
					this.$loader.loader('pause');
					currentView = (this.options.defaultView !== -1) ? this.options.defaultView
					: this.$views.find('label.active input').val();
				...
				Repeater.prototype = {
					...
					getDataOptions : function(options, callback) {
						...
						if (!this.infiniteScrollingEnabled) {
							opts.pageSize = parseInt(this.$pageSize
								.selectlist('selectedItem').value, 10);
						}
					...
					infiniteScrolling : function(enable, options) {
						var itemization = this.$element
						.find('.repeater-itemization');
						var pagination = this.$element
						.find('.repeater-pagination');
						var cont, data;
					...
					infiniteScrollPaging : function(data, options) {
						var end = (this.infiniteScrollingEnd !== true) ? this.infiniteScrollingEnd
						: undefined;
						var page = data.page;
						...
						if ((this.currentPage + 1) >= pages) {
							this.infiniteScrollingCont.infinitescroll('end',
								end);
						}
					},
					...
					initViews : function(callback) {
						...
						var init = function(index) {
							...
							if (views[index].initialize) {
								views[index].initialize.call(this, {},
									function() {
										next();
									});
							} else {
					...
					pagination : function(data) {
						...
						if (pages <= this.options.dropPagingCap) {
							...
							dropMenu = this.$primaryPaging
							.find('.dropdown-menu');
							dropMenu.empty();
							for (i = 0; i < pages; i++) {
								l = i + 1;
								dropMenu.append('<li data-value="' + l
									+ '"><a href="#">' + l + '</a></li>');
							}
							this.$primaryPaging.find('input.form-control').val(
								this.currentPage + 1);
						} else {
					...
					render : function(options) {
						...
						var start = function() {
							options.preserve = (options.preserve !== undefined) ? options.preserve
							: !viewChanged;
							self.clear(options);
							if (!self.infiniteScrollingEnabled
								|| (self.infiniteScrollingEnabled && viewChanged)) {
...
this.currentView = options.changeView;
this.$element.attr('data-currentview',
	this.currentView);
viewChanged = true;
if (this.infiniteScrollingEnabled) {
...
resize : function() {
	...
	if (staticHeight !== undefined) {
		...
		height = ((staticHeight === 'true' || staticHeight === true) ? this.$element
			.height()
			: parseInt(staticHeight, 10))
		- this.$element.find('.repeater-header')
		.outerHeight()
		- this.$element.find('.repeater-footer')
		.outerHeight()
		- parseInt(this.$viewport
			.css('margin-bottom'), 10)
		- parseInt(
			this.$viewport.css('margin-top'),
			10);
	...
	var start = function(args, cb) {
		...
		var callbacks = {
			...
			render : function(resp) {
				var action = (resp && resp.action) ? resp.action
				: 'append';
			...
			after : function(resp) {
				...
				if (renderer.nested && !skipNested) {
					cont = $(item);
					cont = (cont.attr('data-container') === 'true') ? cont
					: cont
					.find('[data-container="true"]:first');
					if (cont.length < 1) {
						cont = container;
					}
					loopNested(cont, 0);
		...
		pageIncrement : null
				...
				$.fn.repeater = function(option) {
					...
					var $set = this.each(function() {
						...
						if (!data)
							$this.data('repeater', (data = new Repeater(this,
								options)));
						if (typeof option === 'string')
							methodReturn = data[option].apply(data, args);
					});
				...
				$.fn.repeater.defaults = {
					...
					defaultView : -1, // should be a string value. -1 means it
										// will grab the active view from the
										// view controls
										dropPagingCap : 10,
										staticHeight : -1
				// normally true or false. -1 means it will look for
				// data-staticheight on the element
			};

				// views object contains keyed list of view plugins, each an
				// object with following optional parameters:
				// {
				// initialize: function(){},
				// selected: function(){},
				// renderer: {}
				...
				// index: int, (only there if repeat was set. current item
				// index)
				// item: str or jQuery object, (only there if rendered function
				// returned item)
				// subset: {}, (only there if repeat was set. subset of data
				// being repeated on)
				// }
				$.fn.repeater.views = {};
			...
			(function($) {
				...
				if ($.fn.repeater) {
					...
					$.fn.repeater.Constructor.prototype.clearSelectedItems = function() {
						...
						this.$canvas.find('.repeater-list-items tr.selected')
					...
					$.fn.repeater.Constructor.prototype.setSelectedItems = function(
						items, force) {
						...
						for (i = 0; i < l; i++) {
							if (items[i].index !== undefined) {
								$item = this.$canvas
								.find('.repeater-list-items tr:nth-child('
									+ (items[i].index + 1) + ')');
								if ($item.length > 0) {
									selectItem($item, items[i].selected);
								}
							} else if (items[i].property !== undefined
								&& items[i].value !== undefined) {
								// lint demanded this function not be within
								// this loop
								this.$canvas.find('.repeater-list-items tr')
								.each(eachFunc);
							}
						}
					};

					$.fn.repeater.defaults = $.extend({},
						$.fn.repeater.defaults, {
					...
					$.fn.repeater.views.list = {
						...
						renderer: {
							complete : function(helpers, callback) {
								...
								if (!this.options.list_columnSyncing
									|| (helpers.data.items.length < 1)) {
									callback();
							} else {
								$header = this.$element
								.find('.repeater-list-header:first');
								$items = this.$element
								.find('.repeater-list-items:first');
						...
						nested: [ {
							complete : function(helpers, callback) {
								...
								if (!this.options.list_columnSizing
									|| this.list_columnsSame) {
									callback();
							} else {
								i = 0;
								taken = 0;
								...
								l = auto.length;
								if (l > 0) {
									newWidth = Math .floor((this.$canvas.width() - taken));
									for (i = 0; i < l; i++) {
										if (!auto[i].last) {
											auto[i].col
											.outerWidth(newWidth);
						...
						render : function(helpers, callback) {
							...
							if (sortable) {
								...
								.on(
									'click',
									function() {
										self.list_sortProperty = (typeof sortable === 'string') ? sortable
										: subset[index].property;
										if ($item
											.hasClass('sorted')) {
											if ($span
												.hasClass(chevUp)) {
...
if (subset[index].sortDirection === 'asc'
	|| subset[index].sortDirection === 'desc') {
	helpers.container
.find('td')
.removeClass(
	'sorted');
$item
.addClass('sortable sorted');
if (subset[index].sortDirection === 'asc') {
...
this.list_sortProperty = (typeof sortable === 'string') ? sortable
: subset[index].property;
}
...
repeat : 'data.columns'
	...
	after : function(helpers, callback) {
		...
		var header = canvas
		.find('.repeater-list-header');
		if (this.staticHeight) {
			helpers.item.height(canvas
				.height()
				- header.outerHeight());
		}
		callback();
	...
	render : function(helpers, callback) {
		...
		if (helpers.data.items.length < 1) {
			...
			$item
			.find(
				'.repeater-list-items tr.empty:first')
			.remove();
		}
		callback(obj);
	},
...
(function($) {
						...
						renderer: {
							render : function(helpers, callback) {
								...
								if (helpers.data.items.length < 1) {
									...
									$empty
									.append(this.options.thumbnail_noItemsHTML);
									$item.append($empty);
								} else {
							...
							nested : [ {
								after : function(helpers, callback) {
									...
									if (this.options.thumbnail_itemRendered) {
										this.options.thumbnail_itemRendered(
											obj, function() {
												callback();
											});
									} else {
								...
								render : function(helpers, callback) {
									callback({
										item : '<div class="thumbnail repeater-thumbnail" style="background: '
										+ helpers.subset[helpers.index].color
										+ ';"><img height="75" src="'
										+ helpers.subset[helpers.index].src
										+ '" width="65">'
										+ helpers.subset[helpers.index].name
										+ '</div>'
									});
								},
								repeat : 'data.items'
			...
			(function($) {
				...
				/*
				 * Fuel UX Scheduler https://github.com/ExactTarget/fuelux
				 * 
				 * Copyright (c) 2014 ExactTarget Licensed under the MIT
				 * license.
				 */
				...
				var Scheduler = function(element, options) {
					...
					this.$element = $(element);
					this.options = $.extend({}, $.fn.scheduler.defaults,
						options);

					// cache elements
					this.$startDate = this.$element
					.find('.start-datetime .start-date');
					this.$startTime = this.$element
					.find('.start-datetime .start-time');

					this.$timeZone = this.$element
					.find('.timezone-container .timezone');
					this.$timeZone.selectlist();

					this.$repeatIntervalPanel = this.$element
					.find('.repeat-every-panel');
					this.$repeatIntervalSelect = this.$element
					.find('.repeat-options');

					this.$repeatIntervalSpinbox = this.$element
					.find('.repeat-every');
					this.$repeatIntervalTxt = this.$element
					.find('.repeat-every-text');
					...
					// panels
					this.$recurrencePanels = this.$element
					.find('.repeat-panel');

					// initialize sub-controls
					this.$repeatIntervalSelect.on('changed.fu.selectlist', $
						.proxy(this.repeatIntervalSelectChanged, this));
					this.$endSelect.on('changed.fu.selectlist', $.proxy(
						this.endSelectChanged, this));
					...
					this.$element.find(
						'.repeat-days-of-the-week .btn-group .btn').on(
						'change', function(e, data) {
							self.changed(e, data, true);
						});
						this.$element.find('.combobox').on('changed',
							$.proxy(this.changed, this));
						this.$element.find('.datepicker').on('changed',
							$.proxy(this.changed, this));
						this.$element.find('.selectlist').on(
							'changed.fu.selectlist',
							$.proxy(this.changed, this));
						this.$element.find('.spinbox').on('changed',
							$.proxy(this.changed, this));
						this.$element.find(
							'.repeat-monthly .radio, .repeat-yearly .radio')
						.on('change', $.proxy( this.changed, this ) );
					...
					getValue : function() {
						...
						// BYMONTH = when picking months (Jan,Feb,March) - note
						// the values should be 1,2,3...
						// BYMONTHDAY = when picking days of the month
						// (1,2,3...)
						// BYSETPOS = when picking
						// First,Second,Third,Fourth,Last (1,2,3,4,-1)

						var interval = this.$repeatIntervalSpinbox
						.spinbox('value');
						var pattern = '';
						var repeat = this.$repeatIntervalSelect
						.selectlist('selectedItem').value;
						var startTime = this.$startTime
						.combobox('selectedItem').text.toLowerCase();
						...
						getFormattedDate = function(dateObj, dash) {
							...
							fdate += dash;
							item = dateObj.getMonth() + 1; // because 0
															// indexing makes
															// sense when
															// dealing with
															// months /sarcasm
														...
														startTime = $
														.trim(
															startTime.replace(/am/g, '').replace(
																/pm/g, '')).split(':');
														...
														startDateTime += (startTime[0] < 10) ? '0'
														+ startTime[0] : startTime[0];
														startDateTime += ':';
														startDateTime += (startTime[1] < 10) ? '0'
														+ startTime[1] : startTime[1];

														startDateTime += (timeZone.offset === '+00:00') ? 'Z'
														: timeZone.offset;
														...
														if (repeat === 'none') {
															...
															this.$element
															.find(
																'.repeat-days-of-the-week .btn-group input:checked')
															.each(function() {
															...
															pattern += 'INTERVAL=' + interval + ';';
															type = this.$element.find(
																'input[name=repeat-monthly]:checked').val();

															if (type === 'bymonthday') {
																day = parseInt(this.$element.find(
																	'.repeat-monthly-date .selectlist')
																.selectlist('selectedItem').text, 10);
																pattern += 'BYMONTHDAY=' + day + ';';
															} else if (type === 'bysetpos') {
																days = this.$element.find('.month-days')
																.selectlist('selectedItem').value;
																pos = this.$element.find('.month-day-pos')
																.selectlist('selectedItem').value;
																pattern += 'BYDAY=' + days + ';';
															...
															pattern += 'FREQ=YEARLY;';
															type = this.$element.find(
																'input[name=repeat-yearly]:checked').val();

															if (type === 'bymonthday') {
																month = this.$element.find(
																	'.repeat-yearly-date .year-month')
																.selectlist('selectedItem').value;
																day = this.$element.find('.year-month-day')
																.selectlist('selectedItem').text;
																pattern += 'BYMONTH=' + month + ';';
																pattern += 'BYMONTHDAY=' + day + ';';
															} else if (type === 'bysetpos') {
																days = this.$element.find('.year-month-days')
																.selectlist('selectedItem').value;
																pos = this.$element.find('.year-month-day-pos')
																.selectlist('selectedItem').value;
																month = this.$element.find(
																	'.repeat-yearly-day .year-month')
																.selectlist('selectedItem').value;
						...
						// if both UNTIL and COUNT are not specified, the
						// recurrence will repeat forever
						// http://tools.ietf.org/html/rfc2445#section-4.3.10
						if (repeat !== 'none') {
							if (end === 'after') {
								duration = 'COUNT='
								+ this.$endAfter.spinbox('value') + ';';
							} else if (end === 'date') {
								duration = 'UNTIL='
					...
					// (None, Hourly, Daily, Weekdays, Weekly, Monthly, Yearly
						repeatIntervalSelectChanged : function(e, data) {
							...
							if (!data) {
								selectedItem = this.$repeatIntervalSelect
								.selectlist('selectedItem');
								val = selectedItem.value;
						...
						switch (val.toLowerCase()) {
							...
							this.$repeatIntervalPanel.attr('aria-hidden',
								'false');
							break;
							default:
						...
						this.$element.find('.repeat-' + val)
						.removeClass('hide');
						this.$element.find('.repeat-' + val).attr(
							'aria-hidden', 'false');
					...
					setValue : function(options) {
						...
						if (options.startDateTime) {
							...
							if (temp[1]) {
								...
								minutes = (temp[1][1]) ? parseInt(
									temp[1][1].split('+')[0].split('-')[0]
									.split('Z')[0], 10) : 0;
						...
						if (options.recurrencePattern) {
							recur = {};
							temp = options.recurrencePattern.toUpperCase()
							.split(';');
							...
							this.$element.find('.repeat-monthly input')
							.removeClass('checked');
							if (recur.BYMONTHDAY) {
								temp = this.$element
								.find('.repeat-monthly-date');
								temp.find('input').addClass('checked');
								temp.find('.select').selectlist(
									'selectByValue', recur.BYMONTHDAY);
							} else if (recur.BYDAY) {
								temp = this.$element
								.find('.repeat-monthly-day');
								temp.find('input').addClass('checked');
								if (recur.BYSETPOS) {
									temp.find('.month-day-pos')
									.selectlist('selectByValue',
										recur.BYSETPOS);
								}
								temp.find('.month-days').selectlist(
									'selectByValue', recur.BYDAY);
							}
							item = 'monthly';
						} else if (recur.FREQ === 'YEARLY') {
							this.$element.find('.repeat-yearly input')
							.removeClass('checked');
							if (recur.BYMONTHDAY) {
								temp = this.$element
								.find('.repeat-yearly-date');
								temp.find('input').addClass('checked');
								if (recur.BYMONTH) {
									temp.find('.year-month').selectlist(
										'selectByValue', recur.BYMONTH);
								}
								temp.find('.year-month-day').selectlist(
									'selectByValue', recur.BYMONTHDAY);
							} else if (recur.BYSETPOS) {
								temp = this.$element
								.find('.repeat-yearly-day');
								temp.find('input').addClass('checked');
								temp.find('.year-month-day-pos')
								.selectlist('selectByValue',
									recur.BYSETPOS);
								if (recur.BYDAY) {
									temp.find('.year-month-days')
									.selectlist('selectByValue',
										recur.BYDAY);
								}
								if (recur.BYMONTH) {
						...
						if (recur.COUNT) {
							this.$endAfter.spinbox('value', parseInt(
								recur.COUNT, 10));
							this.$endSelect.selectlist('selectByValue',
								'after');
						} else if (recur.UNTIL) {
							temp = recur.UNTIL;
							if (temp.length === 8) {
						...
						if (recur.INTERVAL) {
							this.$repeatIntervalSpinbox.spinbox('value',
				...
				toggleState : function(action) {
					...
					this.$element
					.find('.repeat-days-of-the-week .btn-group')[action]
					('disabled');
				},
				...
				$.fn.scheduler = function(option) {
					...
					var $set = this.each(function() {
						...
						if (!data)
							$this.data('scheduler', (data = new Scheduler(this,
								options)));
						if (typeof option === 'string')
							methodReturn = data[option].apply(data, args);
					});
				...
				$('body').on('mousedown.scheduler.data-api', '.scheduler',
					function() {
						var $this = $(this);
